...
Use mocked transports for automated tests. For UI work, inspect demo/live labeling, custom checks, threshold changes, key removal, error states, keyboard controls, themes, and narrow layouts. Also verify replacement Undo (including command-palette actions), inline check polarity previews, nested probability details, and the narrow-screen writing/verdict navigation with evidence return focus. Verify custom-check editing/cancel/preview and removal Undo, definition-sensitive stale results, delayed and failed evidence lookup after visible primary verdicts, duplicate-run prevention, and consecutive-run comparisons (including mode/definition exclusions and failed-run recovery). Draft recovery and previous-run comparison history must remain in memory rather than browser storage. Review the diff for sensitive text even when the scanner passes.
